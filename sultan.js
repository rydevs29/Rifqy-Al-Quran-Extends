/* ==============================================================
   SULTAN.JS - KUMPULAN FITUR PREMIUM & TRACKER SINKRON
   ============================================================== */
window.startVoiceSearch = function() { const SR = window.SpeechRecognition || window.webkitSpeechRecognition; if (SR) { const rec = new SR(); rec.lang = 'id-ID'; rec.start(); const inp = document.getElementById('search-input'); inp.placeholder = "Mendengarkan..."; rec.onresult = e => { inp.value = e.results[0][0].transcript; window.filterSurah(); inp.placeholder = "Cari Surat..."; }; rec.onerror = () => { alert("Suara tidak jelas."); inp.placeholder = "Cari Surat..."; }; } else alert("Tidak didukung."); };
...
window.startCompassReal = function() {
    ...
    if (navigator.geolocation && (window.DeviceOrientationEvent || window.DeviceMotionEvent)) {
        navigator.geolocation.getCurrentPosition(pos => {
            const lat = pos.coords.latitude; const lng = pos.coords.longitude; const kaabaLat = 21.422487; const kaabaLng = 39.826206;
            ...
            const distance = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)); document.getElementById('qibla-distance').innerText = `${distance.toFixed(0)} km`;
            ...
            let qiblaBearing = Math.atan2(y, x) * 180 / Math.PI; qiblaBearing = (qiblaBearing + 360) % 360;
            window.addEventListener('deviceorientation', (e) => { 
                let c = e.webkitCompassHeading || Math.abs(e.alpha - 360); 
                if (c) { 
                    const compassUI = document.getElementById('compass-ring'); compassUI.style.transform = `rotate(${qiblaBearing - c}deg)`; 
                    document.getElementById('qibla-degree').innerText = `${c.toFixed(0)}°`;
                    const diff = Math.abs(c - qiblaBearing);
                    if (diff < 5 || diff > 355) { document.getElementById('qibla-status').innerHTML = `<i class="fas fa-check-circle"></i> Tepat Kiblat!`; document.getElementById('qibla-status').style.color = "#10b981"; if(navigator.vibrate) navigator.vibrate(30); } else { document.getElementById('qibla-status').innerHTML = "Putar HP Anda..."; document.getElementById('qibla-status').style.color = "#f59e0b"; }
                } 
            }, true); window.compassActive = true; 
...
window.switchDoaTab = function(tabName) {
    ...
    if(tabName === 'umum') { tabU.style.borderBottom = "3px solid var(--primary-color)"; tabU.style.color = "var(--primary-color)"; tabT.style.borderBottom = "3px solid transparent"; tabT.style.color = "var(--text-muted)"; contU.classList.remove('hidden'); contT.classList.add('hidden'); } 
    else { tabT.style.borderBottom = "3px solid var(--primary-color)"; tabT.style.color = "var(--primary-color)"; tabU.style.borderBottom = "3px solid transparent"; tabU.style.color = "var(--text-muted)"; contT.classList.remove('hidden'); contU.classList.add('hidden'); window.loadTahlil('singkat'); }
};
window.renderDoaList = function(data) {
    const cont = document.getElementById('doa-container'); if(!cont) return;
    cont.innerHTML = data.map((d, i) => `<div class="doa-item"><div class="doa-header" onclick="window.toggleDoa(${i})"><span>${d.t}</span><i class="fas fa-chevron-down" id="di-icon-${i}"></i></div><div class="doa-body" id="di-body-${i}"><div class="p-3"><p class="font-arab text-right mb-2 text-primary" style="font-size:24px;">${d.a}</p><p class="small text-primary mb-2" style="font-style:italic;">${d.l}</p><p class="small text-muted">${d.i}</p></div></div></div>`).join('');
};
window.filterDoa = function() { const q = document.getElementById('search-doa').value.toLowerCase().trim(); const filtered = doaDatabase.filter(d => d.t.toLowerCase().includes(q) || d.i.toLowerCase().includes(q)); window.renderDoaList(filtered); };
window.toggleDoa = function(idx) { const b = document.getElementById(`di-body-${idx}`); const icon = document.getElementById(`di-icon-${idx}`); if(b.style.maxHeight === '0px' || b.style.maxHeight === '') { b.style.maxHeight = '500px'; icon.classList.replace('fa-chevron-down', 'fa-chevron-up'); } else { b.style.maxHeight = '0px'; icon.classList.replace('fa-chevron-up', 'fa-chevron-down'); } };
...
window.loadTahlil = function(type) {
    ...
    if(type === 'singkat') { btnS.classList.replace('btn-outline-primary','btn-primary'); btnL.classList.replace('btn-primary','btn-outline-primary'); } else { btnL.classList.replace('btn-outline-primary','btn-primary'); btnS.classList.replace('btn-primary','btn-outline-primary'); }
    let html = (type === 'singkat') ? `<div class="bg-light p-3 border-radius mb-3 text-center"><h4 class="font-bold text-primary mb-2">Tahlil Dzikir</h4><p class="font-arab text-primary mb-2" style="font-size:28px;">لَا إِلَهَ إِلَّا اللهُ</p><p class="small text-muted font-bold">Laa ilaaha illallah</p></div>` : `<div class="bg-light p-3 border-radius mb-3"><h4 class="font-bold text-primary mb-3 text-center border-bottom pb-2">Tahlil & Doa Arwah (Lengkap)</h4><p class="font-bold text-success mb-1 small">1. Membaca Al-Fatihah</p><p class="font-arab text-right mb-2" style="font-size: 24px;">بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ...</p></div>`;
...
window.initTracker = function() {
    window.renderAsmaulHusna(); const tList = document.getElementById('tracker-list'); if(!tList) return;
    tList.innerHTML = Object.keys(window.trackerData).map(k => `<div class="tracker-item"><span>${k}</span><input type="checkbox" onchange="window.updateTracker('${k}', this.checked)" ${window.trackerData[k] ? 'checked' : ''}></div>`).join('');
    window.updateTrackerProgress();
};
window.updateTracker = function(k, val) { window.trackerData[k] = val; window.syncStorage.setItem('rTracker', JSON.stringify(window.trackerData)); window.updateTrackerProgress(); };
window.updateTrackerProgress = function() {
    ...
    const pct = Math.round((checked / keys.length) * 100); const prog = document.getElementById('tracker-progress'); const stat = document.getElementById('tracker-status');
    if(prog) prog.style.width = `${pct}%`; if(stat) stat.innerText = `Progres Hari Ini: ${pct}%`;
};
